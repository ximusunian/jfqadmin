  ...
  <div :class="wrapper">
    <template v-if="type != 'mobile'">
      <select
        ...
        :class="(disabled || provinceDisabled) ? 'disabled-color':''"
      >
      ...
      <template v-if="!onlyProvince">
        <select
          ...
          :class="(disabled || cityDisabled) ? 'disabled-color':''"
        >
        ...
        <select
          ...
          :class="(disabled || areaDisabled) ? 'disabled-color':''"
        >
      ...
      <div :class="addressHeader">
        <ul>
          <li
            :class="{'active': tab === 1}"
            @click="resetProvince"
          >{{ currentProvince && !staticPlaceholder ? currentProvince : placeholders.province }}</li>
          <template v-if="!onlyProvince">
            <li
              v-if="showCityTab"
              :class="{'active': tab === 2}"
              @click="resetCity"
            >{{ currentCity && !staticPlaceholder ? currentCity : placeholders.city }}</li>
            <li
              v-if="showAreaTab && !hideArea"
              :class="{'active': tab === 3}"
            >{{ currentArea && !staticPlaceholder ? currentArea : placeholders.area }}</li>
          </template>
        </ul>
      </div>
...
.change .distpicker-address-wrapper select{
  color: red !important;
}
