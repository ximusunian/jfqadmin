<template>
  <div class="app-container">
    ...
    <el-button
      :disabled="!permissionList.Users_Admin_List_Add"
      size="mini"
      type="primary"
    ...
    >
      ...
      <el-table-column label="数据权限">
        <template slot-scope="scope">
          <span>{{ scope.row.dataRoleNames }}</span>
      ...
      <el-table-column label="操作" width="450">
        <template slot-scope="scope">
          <el-button
            :disabled="!permissionList.Users_Admin_List_UpdRole"
            size="mini"
            type="primary"
            @click="showAuthrizeDialog(scope.$index, scope.row)"
          >功能权限</el-button>
          <el-button
            :disabled="!permissionList.Users_Admin_List_UpdRole"
            size="mini"
            type="primary"
            @click="showDataRoleDialog(scope.$index, scope.row)"
          >数据权限</el-button>
          <el-button
            :disabled="!permissionList.Users_Admin_List_Upd"
            size="mini"
            type="primary"
            @click="modifyUserInfo(scope.$index,scope.row)"
          >修改</el-button>
          <el-button
            :disabled="!permissionList.Users_Admin_List_MdfPwd"
            size="mini"
            type="success"
            @click="modifyPassword(scope.$index,scope.row)"
          >修改密码</el-button>
          <el-button
            :disabled="!permissionList.Users_Admin_List_Del"
            size="mini"
            type="danger"
            @click="deleteUser(scope.$index, scope)"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <Pagination
      v-show="totalCount>0"
      :total="totalCount"
      :page.sync="inputFilters.pageIndex"
      :limit.sync="inputFilters.pageSize"
      @pagination="selectPage"
    />

    <el-dialog title="修改功能权限" :visible.sync="isShowAuthrizeDialog">
      <el-select
        v-model="curAuthorizationList"
        multiple
        placeholder="请选择用户组权限"
        :change="selectAuthrizationType(curAuthorizationList)"
      >
        <el-option
    ...
    <el-dialog title="修改数据权限" :visible.sync="isShowDataRoleDialog">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          type="primary"
          v-loading="dataroleuploadloading"
          @click="updateUserDataRole()"
        >确 定</el-button>
      </div>
    </el-dialog>
    ...
    <Password
      ...
      :is-show-password-dialog="isShowPasswordDialog"
      @isChangePWSuccess="resetpwdres"
    />
    <DialogBase :dialog-info="dialogInfo" @confirmHandle="confirmHandle" />
    <EditUser
      :is-new-user="curUserInfo.NewUser"
      :is-partent-show-user-dialog="isShowUserDialog"
      @isAddUserSuccess="addUserSuccess"
...
import { getuserlist, deleteuser, modifyuserrole } from "../../api/user.js";
...
import { getrolebyuser } from "../../api/role.js";
import { isEmpty, formatTime, showNotification } from "../../utils/index.js";
  ...
  methods: {
    ...
    showDataRoleDialog(index, row) {
      ...
      if (this.allDataRoleList.length > 0) {
        if (row.dataRoleNames) {
          var roleNameArr = row.dataRoleNames.split(";");
          roleNameArr.forEach(item => {
            this.allDataRoleList.find(a => {
              if (a.displayName == item) {
                this.curDataRoleList.push(a.id);
              }
            });
          });
        } else {
    ...
    showAuthrizeDialog(index, row) {
      ...
      // })
      this.curAuthorizationList = row.roleNames.split(";");
    ...
    updateUserDataRole() {
      ...
      updateuserdatarole({ userid: userId }, passItems)
        .then(res => {
    ...
    setPermisson() {
      ...
      this.permissionList.Users_Admin_List_Add = arr.includes(
        "Users_Admin_List_Add"
      );
      this.permissionList.Users_Admin_List_Upd = arr.includes(
        "Users_Admin_List_Upd"
      );
      this.permissionList.Users_Admin_List_Del = arr.includes(
        "Users_Admin_List_Del"
      );
      this.permissionList.Users_Admin_List_UpdRole = arr.includes(
        "Users_Admin_List_UpdRole"
      );
      this.permissionList.Users_Admin_List_MdfPwd = arr.includes(
        "Users_Admin_List_MdfPwd"
      );
    },
    resetpwdres() {
